...
describe('packaging artifact verification', () => {
  ...
  it('resolves Electron extract-zip to the patched symlink guard', async () => {
    ...
    expect(source).toContain('const resolvedLink = path.resolve(path.dirname(dest), link)')
    expect(source).toContain('path.isAbsolute(link)')
    expect(source).toContain('await fs.symlink(link, dest)')
    expect(source).not.toContain('Symlink entries are not supported')
  })
})
