...
export const contextRecipeSchema = z.object({ id: z.string().min(1).max(100), maxTokens: z.number().int().positive().max(200_000), includeSelection: z.boolean().default(true), entityLimit: z.number().int().nonnegative().max(100).default(20), semanticLimit: z.number().int().nonnegative().max(20).default(5), recencyLimit: z.number().int().nonnegative().max(20).default(3) })
export type ContextRecipe = z.input<typeof contextRecipeSchema>
export type ContextLayer = 'pinned' | 'structured' | 'recency' | 'semantic'
...
export interface ContextRetrievalTrace { query: string; selectionIncluded: boolean; method?: 'embedding' | 'fts'; candidateCounts: { pinned: number; structured: number; recency?: number; semantic: number }; selectedSources: string[]; omittedSources: string[] }
...
export interface ContextReplayResult {
  ...
  compatibility: {
    ...
    fromRetrievalVersion: number
    fromProjectSchemaVersion: number
