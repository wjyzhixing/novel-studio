...
export interface WorkflowRun { id: string; workflowId: string; relPath?: string; sceneId?: string; relPathSource?: 'runtime' | 'legacy-node-output'; relPathRecovery?: 'unavailable'; status: 'running' | 'waiting_human' | 'succeeded' | 'failed' | 'cancelled'; nodes: Record<string, NodeRun>; outputs: Record<string, unknown>; createdAt: string; updatedAt: string }
export interface NodeExecutionContext { nodeId: string; input: unknown; outputs: Record<string, unknown>; config: Record<string, unknown>; idempotencyKey: string; inheritedContext?: unknown; signal: AbortSignal }
...
export interface RuntimeOptions { runId?: string; relPath?: string; sceneId?: string; variables?: Record<string, unknown>; retry?: number; retryForNode?: (node: Workflow['nodes'][number]) => number; timeoutMs?: number; signal?: AbortSignal; initialState?: WorkflowRun; resumeInput?: unknown; resolveContext?: (outputs: Record<string, unknown>) => unknown; persist?: (state: WorkflowRun) => Promise<void> | void }
