...
export function registerIpc(
  ...
  deps: { pickDirectory(): Promise<string | null>; pickArchiveSave(): Promise<string | null>; pickArchiveOpen(): Promise<string | null>; pickTextImport(): Promise<string | null>; pickImageImport(): Promise<string | null>; pickExportSave(format: import('../shared/chapter').ExportFormat): Promise<string | null>; pickDiagnosticsSave(): Promise<string | null> }
): void {
  ...
  handle(IPC.projectPickTextImport, () => deps.pickTextImport())
  handle(IPC.projectPickExportSave, (format) => deps.pickExportSave(parseOrThrow(z.enum(exportFormats), format)))
  ...
  handle(IPC.chapterExport, (input) => { const value = parseOrThrow(z.object({ format: z.enum(exportFormats), destination: z.string().min(1).max(4_000) }), input); return chapterService.exportAll(value.format, value.destination) })
